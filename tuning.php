...
$currentPage = 'tuning.php';

if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true) {
    header("Location: index.php?trigger_login=true&redirect=" . urlencode(basename($_SERVER['PHP_SELF'])));
    exit;
}
...
try {
    $db = new Database();
    $conn = $db->connect();
    $lines = $conn->query("SELECT LineID, LineName FROM ProductionLines ORDER BY LineID")->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
                            ...
                            <div class="relative">
                                <select id="line_id" name="line_id" required class="w-full bg-slate-950 border border-slate-700 text-white rounded-xl px-4 py-3 appearance-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all cursor-pointer hover:bg-slate-800">
                                    ...
                                    <?php foreach ($lines as $line): ?>
                                        <option value="<?= htmlspecialchars((string)$line['LineID']) ?>"><?= htmlspecialchars($line['LineName']) ?></option>
                                    <?php endforeach; ?>
                                </select>
