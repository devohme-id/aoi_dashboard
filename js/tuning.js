class SearchableSelect {
  ...
  renderUI() {
    ...
    this.trigger.className =
      "w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 flex justify-between items-center cursor-pointer hover:border-indigo-500 transition-all group select-none shadow-sm";
    ...
    this.dropdown.className =
      "absolute top-full left-0 w-full mt-2 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 hidden overflow-hidden ring-1 ring-white/5 origin-top scale-95 opacity-0 transition-all duration-200";
  ...
  syncFromOriginal() {
    ...
    if (options.length <= 1) {
      ...
      Array.from(options).forEach((opt) => {
        ...
        div.className = `custom-option px-3 py-2.5 rounded-lg cursor-pointer text-sm mb-1 flex items-center justify-between group transition-all ${
          isSelected
  ...
  setLoading(isLoading) {
    if (isLoading) {
      this.trigger.querySelector("span").innerHTML =
        '<div class="flex items-center gap-2"><div class="w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div><span class="text-slate-400">Loading...</span></div>';
...
class TuningApp {
  ...
  init() {
    this.lineSelectUI = new SearchableSelect(
      "#line_id",
      "-- Select Production Line --",
      (id) => this.handleLineChange(id)
    );
    this.assemblySelectUI = new SearchableSelect(
      "#assembly_name",
      "-- Choose Line First --",
      (name) => this.handleAssemblyChange(name)
    );
  ...
  startClock() {
    const update = () => {
      ...
      if (this.el.clock)
        this.el.clock.textContent = now.toLocaleTimeString("id-ID", {
          hour12: false,
        });
      if (this.el.date)
        this.el.date.textContent = `${days[now.getDay()]}, ${now.getDate()} ${
          months[now.getMonth()]
        } ${now.getFullYear()}`;
    };
    update();
  ...
  handleLineChange(lineId) {
    ...
    this.el.preview.badge.textContent = lineId
      ? `LINE ${lineId}`
      : "NO SELECTION";
    this.el.preview.badge.className = `px-3 py-1 rounded-md text-[10px] uppercase font-bold tracking-widest ${
      lineId
  ...
  fetchTuningHistory(lineId, assembly) {
    ...
    fetch("api/get_report_data.php", { method: "POST", body: formData })
      ...
      .then((data) => {
        ...
        this.state.currentHistoryData = [...map.values()].sort(
                ...
                <div class="group flex flex-col border-b border-slate-800/50 pb-2 mb-2 last:border-0 last:mb-0 last:pb-0 hover:bg-slate-800/30 p-2 rounded-lg transition-colors">
                    <div class="flex justify-between items-start">
                        <span class="text-xs font-bold text-indigo-400 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 bg-indigo-500 rounded-full"></span> Cycle #${
                              h.version
                            }
                        </span>
                        <span class="text-[9px] text-slate-500 font-mono">${new Date(
                          h.date
                        ).toLocaleDateString()}</span>
                    </div>
                    <p class="text-[10px] text-slate-300 italic line-clamp-2 mt-1 pl-2 border-l-2 border-slate-700">"${
                      h.notes || "No notes"
                    }"</p>
                    <div class="text-[9px] text-slate-500 mt-1 pl-2">By: <span class="text-slate-400 font-medium">${
                      h.user || "Unknown"
                    }</span></div>
                </div>
              `
    ...
    this.el.preview.container.innerHTML = `
        <div class="space-y-4 animate-[fadeIn_0.3s_ease-out]">
            <div class="bg-slate-950 border ${borderColor} rounded-2xl p-5 shadow-xl relative overflow-hidden">
                ${
                  isCritical
                    ? '<div class="absolute top-0 right-0 p-2"><span class="animate-ping absolute inline-flex h-3 w-3 rounded-full bg-red-400 opacity-75"></span><span class="relative inline-flex rounded-full h-3 w-3 bg-red-500"></span></div>'
                    : ""
                }
                ...
                <div class="flex justify-between items-start mb-6 relative z-10">
                    <div>
                        ...
                        <div class="text-xl font-bold text-white leading-tight tracking-tight">${
                          data.kpi.assembly || "N/A"
                        }</div>
                        <div class="text-xs text-indigo-400 mt-0.5 font-mono">${
                          data.kpi.lot_code || "-"
                        }</div>
                    </div>
