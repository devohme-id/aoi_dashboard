...
<head>
  ...
  <!-- Custom Styles untuk DataTables Dark Mode -->
  <style>
  ...
  <main class="max-w-[1920px] mx-auto p-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-slate-400 text-sm mt-1 ml-14">Analyze historical production performance.</p>
        </div>
    </div>
            ...
            <div class="flex flex-col xl:flex-row gap-6 items-end">
                <div class="flex-grow grid grid-cols-1 md:grid-cols-2 gap-5 w-full xl:w-auto">
                    ...
                    <div class="space-y-1.5 group">
                        <label for="date_range" class="text-xs font-bold text-slate-500 uppercase tracking-wider group-focus-within:text-blue-400 transition-colors flex items-center gap-2">Date Range</label>
                    ...
                    <!-- Line Filter -->
                    <div class="space-y-1.5 group">
                        <label for="line_filter" class="text-xs font-bold text-slate-500 uppercase tracking-wider group-focus-within:text-blue-400 transition-colors flex items-center gap-2">Production Line</label>
                        <div class="relative">
                            ...
                            <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-slate-500"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div>
                        </div>
                    </div>
                </div>
                <!-- Action Buttons -->
                <div class="flex gap-3 w-full xl:w-auto">
                    <button id="view_data" class="flex-1 xl:flex-none px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold rounded-lg shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2 transform active:scale-95">FILTER DATA</button>
                    <button id="export_excel" class="flex-1 xl:flex-none px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold rounded-lg shadow-lg shadow-emerald-500/20 transition-all flex items-center justify-center gap-2 transform active:scale-95">EXPORT EXCEL</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden ring-1 ring-white/5">
        ...
        <div class="p-2">
            <table id="report_table" class="display w-full text-sm text-left border-collapse" width="100%"></table>
        </div>
    </div>
  </main>
